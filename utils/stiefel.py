...
def gen_tangent_samples(N_samples: int, beta: cp.Variable, X: NDArray, seed: int = 42):
    ...
    w0 = gamma.rvs(np.tile(beta.value, (N_samples, 1)), scale=scale, size=(N_samples, beta.shape[0]))

    # Normalize the rows of w
    w = w0 / np.sum(w0, axis=1, keepdims=True)
    ...
    return tangential_samples, w0, w, maxI
...
def gen_stiefel_samples(N_samples: int, rob: NDArray, tau: float = 1e-4, metric_alpha=0.0):
    ...
    tangential_samples, w0, w, maxI = gen_tangent_samples(N_samples, beta, X) 
    ...
    return stiefel_samples, tangential_samples, w0, w, maxI, Deltas, beta
