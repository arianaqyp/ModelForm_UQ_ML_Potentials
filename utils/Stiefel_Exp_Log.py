...
#------------------------------------------------------------------------------
def Stiefel_Log_alg(U0, U1, tau, do_Procrustes=1, do_Cayley=1, do_Sylvester=1):
#------------------------------------------------------------------------------
    ...
    start_op = time.time()
    M = np.dot(U0.T, U1)
    print(f"1. in {time.time() - start_op:.4f} seconds.")
    ...
    Q, N = scipy.linalg.qr(U0orth, overwrite_a=True,\
                                ...
                                pivoting=False,\
                                check_finite=True)
    print(f"2. Compact QR-decomp. in {time.time() - start_op:.4f} seconds.")

    # step 3
    MN = np.concatenate((M,N), axis=0)    
    # orthogonal completion
    start_op = time.time()
    print("Starting orthogonal completion ...")
    ...
    if do_Procrustes:
        print("Procrustes preprocessing ...")
        ...
        print("Completed Procrustes preprocessing.")
        
    V = np.concatenate((MN, V[:,p:2*p]), axis=1)
                                           #          |M  X0|
                                           # now, V = |N  Y0|
    print(f"3. Orthonormal completion in {time.time() - start_op:.4f} seconds.")
    ...
    if check_det:
        ...
        if DetV < 0:
            ...
            V[:,p] = (-1)*V[:,p]
        print("DetV:", DetV)
        print(f"Det. check in {time.time() - start_op:.4f} seconds.")
        print(" ")
    ...
    for k in range(1000):
        ...
        # step 9
        print("k, normC:", k, normC)
        if do_Sylvester:
            print("Solving Sylvester equation ...")
            ...
            print("Completed Sylvester equation.")

        # exponential of updated block
        if do_Cayley:
            print("Using Cayley approximation ...")
            # Cayley approx
            Phi = StAux.Cayley(-C)
            print("Completed Cayley approximation.")
        else:
            # standard matrix exponential
            print("Using standard matrix exponential ...")
            Phi = scipy.linalg.expm(-C)
            print("Completed standard matrix exponential.")

        # step 10: rotate the last p columns
        V[:,p:2*p] = np.dot(V[:,p:2*p],Phi)   # update last p columns
        print("Completed rotation of the last p columns.")
        print(f"Loop iteration {k}, time: {time.time() - start_op:.4f} seconds.")
        print(" ")
    ...
    print("Completed preparation of output.")
